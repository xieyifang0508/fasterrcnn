...
if __name__ == "__main__":
    ...
    #      
    # 在此提供若干参数设置建议，各位训练者根据自己的需求进行灵活调整： （一）从整个模型的预训练权重开始训练： Adam： Init_Epoch = 0，Freeze_Epoch = 50，UnFreeze_Epoch =
    # 100，Freeze_Train = True，optimizer_type = 'adam'，Init_lr = 1e-4。（冻结） Init_Epoch = 0，UnFreeze_Epoch =
    # 100，Freeze_Train = False，optimizer_type = 'adam'，Init_lr = 1e-4。（不冻结） SGD： Init_Epoch = 0，Freeze_Epoch =
    # 50，UnFreeze_Epoch = 150，Freeze_Train = True，optimizer_type = 'sgd'，Init_lr = 1e-2。（冻结） Init_Epoch =
    # 0，UnFreeze_Epoch = 150，Freeze_Train = False，optimizer_type = 'sgd'，Init_lr = 1e-2。（不冻结）
    # 其中：UnFreeze_Epoch可以在100-300之间调整。 （二）从主干网络的预训练权重开始训练： Adam： Init_Epoch = 0，Freeze_Epoch = 50，UnFreeze_Epoch =
    # 100，Freeze_Train = True，optimizer_type = 'adam'，Init_lr = 1e-4。（冻结） Init_Epoch = 0，UnFreeze_Epoch =
    # 100，Freeze_Train = False，optimizer_type = 'adam'，Init_lr = 1e-4。（不冻结） SGD： Init_Epoch = 0，Freeze_Epoch =
    # 50，UnFreeze_Epoch = 150，Freeze_Train = True，optimizer_type = 'sgd'，Init_lr = 1e-2。（冻结） Init_Epoch =
    # 0，UnFreeze_Epoch = 150，Freeze_Train = False，optimizer_type = 'sgd'，Init_lr = 1e-2。（不冻结）
    # 其中：由于从主干网络的预训练权重开始训练，主干的权值不一定适合目标检测，需要更多的训练跳出局部最优解。 UnFreeze_Epoch可以在150-300之间调整，YOLOV5和YOLOX均推荐使用300。
    # Adam相较于SGD收敛的快一些。因此UnFreeze_Epoch理论上可以小一点，但依然推荐更多的Epoch。 （三）batch_size的设置：
    # 在显卡能够接受的范围内，以大为好。显存不足与数据集大小无关，提示显存不足（OOM或者CUDA out of memory）请调小batch_size。 faster rcnn的Batch
    # BatchNormalization层已经冻结，batch_size可以为1
    # 冻结阶段训练参数 此时模型的主干被冻结了，特征提取网络不发生改变 占用的显存较小，仅对网络进行微调 Init_Epoch
    # 模型当前开始的训练世代，其值可以大于Freeze_Epoch，
    # 如设置： Init_Epoch = 60、Freeze_Epoch = 50、UnFreeze_Epoch = 100
    # 会跳过冻结阶段，直接从60代开始，并调整对应的学习率。 （断点续练时使用） Freeze_Epoch
    # 模型冻结训练的Freeze_Epoch (当Freeze_Train=False时失效) Freeze_batch_size
    # 模型冻结训练的batch_size (当Freeze_Train=False时失效)
    Init_Epoch = 0
    Freeze_Epoch = 50
    Freeze_batch_size = 4
    # ------------------------------------------------------------------#
